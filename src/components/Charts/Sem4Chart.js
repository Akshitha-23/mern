...
export default function Sem4Charts() {
  ...
  const labelss = ['ADSJ', 'DBMS', 'CAMC', 'DAA', 'ANN'];
  const url = `http://localhost:5000/sem4/${rollnumber}`;

  const fetchHighestMarks = async (subject) => {
    try {
      const response = await fetch(`http://localhost:5000/sem4/max/${subject}`, {
        method: 'POST',
      });
  ...
  const fetchAverageMarks = async (subject) => {
    try {
      const response = await fetch(`http://localhost:5000/sem4/avg/${subject}`, {
        method: 'POST',
      });
