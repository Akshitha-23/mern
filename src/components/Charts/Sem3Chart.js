...
export default function Sem3Charts() {
  ...
  const fetchHighestMarks = async (subject) => {
    try {
      const response = await fetch(`http://localhost:5000/sem3/max/${subject}`, {
        method: 'POST',
      });
  ...
  const fetchAverageMarks = async (subject) => {
    try {
      const response = await fetch(`http://localhost:5000/sem3/avg/${subject}`, {
        method: 'POST',
      });
